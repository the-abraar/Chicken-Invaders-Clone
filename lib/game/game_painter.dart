...
class GamePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (engine.shake > 0) {
      final dx = (Random().nextDouble() - 0.5) * 10 * engine.shake;
      final dy = (Random().nextDouble() - 0.5) * 10 * engine.shake;
      canvas.save();
  ...
  void _drawBackground(Canvas canvas, Size size) {
    final grad = LinearGradient(
      ...
      colors: [const Color(0xFF060612), const Color(0xFF0D0D2B), const Color(0xFF111122)],
  ...
  void _drawStars(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 80; i++) {
      ...
      _p.color = Colors.white.withOpacity(twinkle * 0.75);
  ...
  void _drawRoad(Canvas canvas, Size size) {
    ...
    // Dashes
    _pFill.color = const Color(0xFFFFDD00).withOpacity(0.5);
  ...
  void _drawSergeant(Canvas canvas, double cx, double cy, Enemy e) {
    ...
    final hatPaint = Paint()..color = e.hatColor.withRed((e.hatColor.red * 0.7).round());
    final hat = Path()
    ...
    // Hat brim
    _pFill.color = Colors.black.withOpacity(0.6);
    canvas.drawRect(Rect.fromLTWH(cx - r * 0.7, cy - r * 0.65, r * 1.4, 3), _pFill);

    // Badge star
    _drawEmoji(canvas, '⭐', cx + r * 0.35, cy - r * 0.9, 9);

    // Face emoji
    _drawEmoji(canvas, e.emoji, cx, cy + 1, 22);
  ...
  void _drawBoss(Canvas canvas) {
    ...
    // Aura
    _pFill.color = Colors.red.withOpacity(0.15 * pulse);
  ...
  void _drawBullets(Canvas canvas) {
    for (final b in engine.bullets) {
      ...
      // Glow
      _pFill.color = Colors.yellow.withOpacity(0.3);
  ...
  void _drawMamlas(Canvas canvas) {
    for (final m in engine.mamlas) {
      ...
      canvas.drawRRect(
        ...
        _pFill);
      // Lines (text effect)
      _pFill.color = const Color(0xFF2244AA).withOpacity(0.6);
      for (int i = 0; i < 4; i++) {
        canvas.drawRect(Rect.fromLTWH(-8, -9.0 + i * 5, 16, 1.5), _pFill);
      }
      // Red stamp
      _pFill.color = Colors.red.withOpacity(0.7);
  ...
  void _drawPlayer(Canvas canvas, Size size) {
    ...
    // Shield glow
    if (engine.shielded) {
      _pFill.color = Colors.cyanAccent.withOpacity(0.2 + 0.15 * sin(t * 8));
      canvas.drawCircle(Offset.zero, 36, _pFill);
      _pStr..color = Colors.cyanAccent.withOpacity(0.6)..strokeWidth = 2;
    ...
    _pFill.color = const Color(0xFFAACCFF).withOpacity(0.7);
    ...
    if (engine.movLeft || engine.movRight) {
      for (int i = 0; i < 3; i++) {
        ...
        _pFill.color = Colors.grey.withOpacity(alpha);
    ...
    if (engine.shootRatio < 0.18) {
      _pFill.color = Colors.yellow.withOpacity(0.5 * (1 - engine.shootRatio / 0.18));
  ...
  void _drawPowerUps(Canvas canvas) {
    for (final p in engine.powerUps) {
      ...
      // Glow ring
      _pFill.color = Colors.cyan.withOpacity(0.25 + 0.1 * sin(t * 6));
  ...
  void _drawExplosions(Canvas canvas) {
    for (final ex in engine.explosions) {
      ...
      for (final part in ex.particles) {
        _pFill.color = part.color.withOpacity(alpha);
  ...
  void _drawFloatingTexts(Canvas canvas) {
    for (final ft in engine.floats) {
      _drawText(canvas, ft.text, ft.x, ft.y, 13, ft.color.withOpacity(ft.opacity.clamp(0, 1)));
    }
  }
  ...
  void _drawHUD(Canvas canvas, Size size) {
    // Top bar backdrop
    _pFill.color = Colors.black.withOpacity(0.55);
    ...
    // Lives (hearts)
    for (int i = 0; i < 3; i++) {
      final alpha = i < engine.lives ? 1.0 : 0.2;
      _drawEmoji(canvas, '❤️', 16 + i * 22.0, 44, 16 * alpha);
    }
    ...
    if (engine.viralCharge > 0) {
      ...
      _pFill.color = charged ? Colors.orange : Colors.orange.withOpacity(0.55);
      ...
      if (charged) {
        _drawText(canvas, '🔥 VIRAL READY – swipe up!', size.width / 2, barY + barH + 9, 10,
            Colors.orange.withOpacity(0.8 + 0.2 * sin(t * 6)));
      }
    }
  }
  ...
  void _drawBossWarning(Canvas canvas, Size size) {
    _pFill.color = Colors.black.withOpacity(0.5);
  ...
  void _drawLevelComplete(Canvas canvas, Size size) {
    _pFill.color = Colors.black.withOpacity(0.3);
    canvas.drawRect(Offset.zero & size, _pFill);
